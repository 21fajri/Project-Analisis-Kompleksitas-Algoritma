...
let iterativeTime = 0;
let recursiveTime = 0;
let peminjamanChartInstance: any = null;
let isSelectionMode = false;
let selectedItems: { id: number; isDigital: boolean }[] = [];
let allNewsData: any[] = [];
let newsPointer = 0;
...
async function processReturn(id: number) {
    if (!confirm("Apakah Anda yakin ingin mengembalikan buku ini?")) return;
    try {
        const response = await fetch(`/api/books/return?id=${id}`, { method: "DELETE" });
        if (response.ok) {
...
(window as any).openReturnModal = openReturnModal;
(window as any).closeReturnModal = closeReturnModal;
(window as any).processReturn = processReturn;
async function processBorrow(id: number, type: string) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            await renderBooks();            
            await renderDigitalBooks();     
            await renderAllBooksSorted();   
            await renderBorrowedList();     
            await updateBorrowedBadge();    
            await updateLoyaltyInfo();      
        }
    } catch (error) {
...
function displayRotatedNews() {
    ...
    if (!container || allNewsData.length === 0) return;
...
async function renderBooks() {
    ...
    try {
        ...
        container.innerHTML = '';
        limitedBooks.forEach((book: Book) => {
...
async function renderDigitalBooks() {
    const container = document.getElementById('scroll-digital-new');
    if (!container) return;
    try {
        const response = await fetch("/api/digital-books");
        const books: Book[] = await response.json();
        const limitedBooks = books.slice(0, 100);
        container.innerHTML = '';
        limitedBooks.forEach((book: Book) => {
...
function createBookCard(book: Book, type: string) {
...
async function renderAllBooksSorted(order: 'asc' | 'desc' = 'asc') {
    ...
    try {
        ...
        const fisik: Book[] = await resFisik.json();
        const digital: Book[] = await resDigital.json();
        let allBooks = [
            ...fisik.map(b => ({ ...b, type: "Physical" })),
            ...digital.map(b => ({ ...b, type: "Digital" }))
        ];
        allBooks.sort((a, b) => {
...
async function confirmDelete(id: number, isDigital: boolean = false) {
...
function setupModal() {
    ...
    openBtnDigital?.addEventListener('click', () => {
...
function showToast(title: string, message: string, duration: number = 3000) {
...
async function resetDigitalDatabase() {
    if (!confirm("Hapus semua data Buku Digital saja?")) return;
    try {
        const response = await fetch("/api/digital-books/delete-all", { method: "DELETE" });
        if (response.ok) {
            showToast("Digital Reset!", "Hanya buku digital yang dikosongkan.");
            await renderDigitalBooks();     
            await renderAllBooksSorted();   
            await renderDashboard();        
            await updateTotalBooksStat();  
...
async function resetDatabase() {
    if (!confirm("PERINGATAN ⚠️ : Semua data buku, daftar pinjaman, dan POIN LOYALITAS akan dihapus permanen!")) return;
    try {
        await fetch("/api/books/delete-all", { method: "DELETE" });
        await fetch("/api/digital-books/delete-all", { method: "DELETE" });
        await fetch("/api/stats/reset-all", { method: "DELETE" });
...
async function resetPhysicalDatabase() {
    if (!confirm("Hapus semua data Buku Fisik saja?")) return;
    try {
        const response = await fetch("/api/books/delete-all", { method: "DELETE" });
        if (response.ok) {
...
(window as any).resetPhysicalDatabase = resetPhysicalDatabase;

async function generate5kData() {
    const btn = document.getElementById('btnGenerateRandom') as HTMLButtonElement;
    if (!btn) return;
    btn.disabled = true;
    const originalContent = btn.innerHTML;
    btn.innerHTML = "Sabar Ya!";
    try {
        const response = await fetch("/api/books/generate", { method: "POST" });
...
async function generateDigitalData() {
    const btn = document.getElementById('btnGenerateDigital') as HTMLButtonElement;
    if (!btn) return;
    btn.disabled = true;
    const originalContent = btn.innerHTML;
    btn.innerHTML = "Sabar Ya!";
    ...
    try {
        const res = await fetch("/api/digital-books/generate", { method: "POST" });
...
(window as any).generateDigitalData = generateDigitalData;
...
async function renderDashboard() {
    try {
        ...
        const canvas = document.getElementById('peminjamanChart') as HTMLCanvasElement;
        const ctx = canvas.getContext('2d');
        if (!ctx) return;
        if (peminjamanChartInstance) peminjamanChartInstance.destroy();
...
const inputCari = document.getElementById('searchInput') as HTMLInputElement;
inputCari?.addEventListener('input', (e) => {
    ...
    semuaBuku.forEach((buku) => {
        const element = buku as HTMLElement;
...
const btnOpen = document.getElementById('btnOpenSidebar') as HTMLButtonElement | null;
const btnClose = document.getElementById('btnCloseSidebar') as HTMLButtonElement | null;
const mainContent = document.getElementById('mainContent') as HTMLElement | null;
const mainFooter = document.getElementById('mainFooter') as HTMLElement | null;
...
function toggleSidebar(): void {
    ...
    if (window.innerWidth > 1024) {
        ...
        mainFooter?.classList.toggle('lg:ml-72');
    }
}

[btnOpen, btnClose, overlay].forEach((el: HTMLElement | null) => {
...
function setupSlider(containerId: string, prevBtnId: string, nextBtnId: string) {
...
async function updateBorrowedBadge() {
    const badge = document.getElementById('borrowed-count-badge');
    if (!badge) return;
    try {
        const response = await fetch("/api/books/borrowed-count");
...
function handleCardClick(id: number, isDigital: boolean, element: HTMLElement) {
...
function toggleDeleteMode(active: boolean) {
    isSelectionMode = active;
    selectedItems = [];
    const deleteCountElem = document.getElementById('delete-count');
    if (deleteCountElem) deleteCountElem.innerText = "0";
...
async function executeBulkDelete() {
    ...
    if (!confirm(`Hapus ${selectedItems.length} buku yang dipilih?`)) return;
...
async function updateTotalBooksStat() {
    const display = document.getElementById('total-buku-display');
    if (!display) return;
    try {
        const response = await fetch("/api/stats/total-books");
        const data = await response.json();
...
async function sendHeartbeat() {
    try {
        await fetch("/api/stats/active-visitors");
    } catch (e) {
...
async function updateActiveVisitors() {
    ...
    if (!display) return;
    try {
        const response = await fetch("/api/stats/active-visitors");
        const data = await response.json();
...
async function renderExternalNews() {
    ...
    try {
        ...
        container.innerHTML = '';
        articles.slice(0, 3).forEach((news: any) => {
            const newsHtml = `
...
async function updateLoyaltyInfo() {
    const pointsDisplay = document.getElementById('loyalty-points');
    const recTimeDisplay = document.getElementById('fib-rec-time');
    const iterTimeDisplay = document.getElementById('fib-iter-time');
    if (!pointsDisplay) return;
    try {
        const response = await fetch("/api/stats/loyalty");
        const data = await response.json();
        pointsDisplay.innerText = data.points.toLocaleString('id-ID');
        if (recTimeDisplay) recTimeDisplay.innerText = data.recursive_sec.toFixed(6) + " s";
        if (iterTimeDisplay) iterTimeDisplay.innerText = data.iterative_sec.toFixed(6) + " s";
...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('btn-toggle-delete')?.addEventListener('click', () => toggleDeleteMode(true));
    document.getElementById('btn-cancel-delete')?.addEventListener('click', () => toggleDeleteMode(false));
    document.getElementById('btn-confirm-delete')?.addEventListener('click', executeBulkDelete);
    document.getElementById('btnResetData')?.addEventListener('click', resetDatabase);
    document.getElementById('btnResetDigital')?.addEventListener('click', resetDigitalDatabase);
    document.getElementById('btnGenerateRandom')?.addEventListener('click', generate5kData);
    document.getElementById('btnGenerateDigital')?.addEventListener('click', generateDigitalData);
    document.getElementById('btnSimpanBuku')?.addEventListener('click', addNewBook);
    ...
    btnSortAsc?.addEventListener('click', () => {
        ...
        btnSortDesc?.classList.remove('bg-white', 'shadow-sm', 'text-indigo-600');
        renderAllBooksSorted('asc');
    });
    btnSortDesc?.addEventListener('click', () => {
        btnSortDesc.classList.add('bg-white', 'shadow-sm', 'text-indigo-600');
        btnSortAsc?.classList.remove('bg-white', 'shadow-sm', 'text-indigo-600');
...
function updateActiveFooter(): void {
